...
public sealed class ServiceReportExporter
{
    public string ExportJson(ServiceComparison comparison, ServiceRollbackPlan? rollbackPlan = null)
    {
        var report = new ServiceReport(
            ...
            comparison.Changes,
            rollbackPlan?.Operations ?? Array.Empty<ServiceRollbackOperation>(),
    ...
    public string ExportHtml(ServiceComparison comparison, ServiceRollbackPlan? rollbackPlan = null)
    {
        ...
        <head>
          <meta charset="utf-8">
          <title>WinLedger Services Report</title>
          <style>
    ...
    public string ExportText(ServiceComparison comparison, ServiceRollbackPlan? rollbackPlan = null)
    {
        return PlainTextReportFormatter.Format(
            "WinLedger Services Report",
            comparison.ComparedAt,
    ...
    private sealed record ServiceReport(
        ...
        IReadOnlyList<ServiceRollbackOperation> RollbackPlan,
