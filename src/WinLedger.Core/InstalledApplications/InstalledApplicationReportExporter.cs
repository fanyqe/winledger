...
public sealed class InstalledApplicationReportExporter
{
    public string ExportJson(
        InstalledApplicationsComparison comparison,
        InstalledApplicationRollbackPlan? rollbackPlan = null)
    {
        var report = new InstalledApplicationReport(
            "1.0",
            ...
            comparison.Changes,
            rollbackPlan?.Operations ?? Array.Empty<InstalledApplicationRollbackOperation>(),
        ...
        InstalledApplicationRollbackPlan? rollbackPlan = null)
    {
        var rows = comparison.Changes.Select(change =>
            ...
            <tr>
              ...
              <td>{Escape(change.Source.ToString())}</td>
        ...
        <head>
          <meta charset="utf-8">
          <title>WinLedger Installed Applications Report</title>
          <style>
        ...
        <body>
          <h1>WinLedger Installed Applications Report</h1>
          <p class="muted">Compared at {{Escape(comparison.ComparedAt.ToString("u"))}}. Rollback operations: {{operationCount}}.</p>
          <table>
            <thead>
              <tr><th>Change</th><th>Source</th><th>Application</th><th>Summary</th><th>Rollback</th></tr>
            </thead>
            <tbody>
    ...
    public string ExportText(InstalledApplicationsComparison comparison, InstalledApplicationRollbackPlan? rollbackPlan = null)
    {
        return PlainTextReportFormatter.Format(
            "WinLedger Installed Applications Report",
            comparison.ComparedAt,
    ...
    private sealed record InstalledApplicationReport(
        ...
        IReadOnlyList<InstalledApplicationRollbackOperation> RollbackPlan,
