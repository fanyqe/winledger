...
internal sealed class FirewallCliCommands(IServiceProvider services)
{
    public async Task<int> CaptureAsync(string[] args)
    {
        if (args.Length < 4)
        {
            Console.Error.WriteLine("Usage: winledger firewall-capture <database> <session-id> <snapshot-name>");
            return 2;
        }
        ...
        var collector = services.GetRequiredService<IFirewallSnapshotCollector>();
        var snapshot = await collector.CaptureAsync(session.Id, args[3], CancellationToken.None)
            .ConfigureAwait(false);
        await store.SaveFirewallSnapshotAsync(snapshot, CancellationToken.None).ConfigureAwait(false);

        Console.WriteLine($"SessionId: {session.Id}");
        Console.WriteLine($"SnapshotId: {snapshot.Id}");
        Console.WriteLine($"FirewallRules: {snapshot.Rules.Count}");
    ...
    public async Task<int> CompareAsync(string[] args)
    {
        if (args.Length < 5)
        {
            Console.Error.WriteLine("Usage: winledger firewall-compare <database> <baseline-snapshot-id> <comparison-snapshot-id> <report-output>");
            return 2;
        }
        ...
        var baseline = await store.GetFirewallSnapshotAsync(Guid.Parse(args[2]), CancellationToken.None)
            .ConfigureAwait(false);
        var comparison = await store.GetFirewallSnapshotAsync(Guid.Parse(args[3]), CancellationToken.None)
            .ConfigureAwait(false);

        if (baseline is null || comparison is null)
        {
            Console.Error.WriteLine("One or both firewall snapshots could not be found.");
            return 3;
        }

        var comparer = services.GetRequiredService<FirewallSnapshotComparer>();
        var clock = services.GetRequiredService<IClock>();
        var result = comparer.Compare(baseline, comparison, clock.UtcNow);
        var plan = services.GetRequiredService<FirewallRollbackPlanner>().CreatePlan(result, clock.UtcNow);
        var exporter = services.GetRequiredService<FirewallReportExporter>();
    ...
    public async Task<int> ApplyRollbackAsync(string[] args)
    {
        if (args.Length < 3)
        {
            Console.Error.WriteLine("Usage: winledger firewall-rollback-apply <report-json> <operation-id|all>");
            return 2;
        }

        var reportJson = await File.ReadAllTextAsync(args[1]).ConfigureAwait(false);
        var report = JsonSerializer.Deserialize<FirewallRollbackReport>(reportJson, WinLedgerJsonSerializer.Options)
        ...
        var plan = new FirewallRollbackPlan(Guid.NewGuid(), Guid.Empty, services.GetRequiredService<IClock>().UtcNow, report.RollbackPlan, []);
